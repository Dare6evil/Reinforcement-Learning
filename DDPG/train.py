...
for run in range(1, 1 + runs):
    ...
    for episode in range(episodes):
        ...
        while True:
            x = (mu - x) * theta + numpy.random.normal(size=env.action_space.shape[0]) * sigma
            action = numpy.array([pi(torch.Tensor(state).to(device)).detach().item()]) + x
            ...
            if batch_size < len(replay_buffer):
                ...
                loss_q = torch.nn.functional.mse_loss(
                     ...
                     * q_target(
                     ...
                     + torch.Tensor(reward_batch).to(device).unsqueeze(1)).detach(),
                    q(
                        torch.cat(
                            (
                                torch.Tensor(numpy.array(action_batch)).to(device),
